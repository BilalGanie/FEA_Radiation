...
class FEA_study:
    def __init__(self,
        ...
        #powerdensity=0.2e-2,
        powerdensity = 0.467e-2,
        periphery_powerdensity = 0.656e-2,
        ...
        self.periphery_powerdensity= periphery_powerdensity
    ...
    def calc_avg_X0(self):
        ...
        def X0_cf(x):
                ...
                x0 = np.ones_like(x) * (2.0 * self.cfthickness / self.xcf)
                left_mask = (x >= 0.0) & (x <= self.edge_clearance)
                right_mask = (
                        (x >= self.model_length - self.edge_clearance)
                        & (x <= self.model_length)
                )

                # Left end: bottom CF removed, top CF remains
                x0[left_mask] = self.cfthickness / self.xcf

                # Right end: top CF removed, bottom CF remains
                x0[right_mask] = self.cfthickness / self.xcf
        ...
        # Total X0 profile
        # --------------------------------------------------
        x = np.linspace(0.0, self.model_length, 200001)
    ...
    def create_geometry(self):
        ...
        active_length = 20.233
        periphery_length = 2.327

        for x0 in self.bot_silicon_x:

                sensor_active = mt.createRectangle(
                        start=[x0, 0],
                        end=[x0 + active_length, self.si_thickness],
                        boundaryMarker=[101,102,103,104],
                        marker=9,
                        area=0.01
                )

                sensor_periphery = mt.createRectangle(
                        start=[x0 + active_length, 0],
                        end=[x0 + active_length + periphery_length, self.si_thickness],
                        boundaryMarker=[101,102,103,104],
                        marker=19,
                        area=0.01
                )
                ...
                glue = mt.createRectangle(
                        ...
                        self.si_thickness + self.si_glue_thickness],
                        boundaryMarker=[5,6,7,8],
                        marker=10,
                        area=0.01
                )

                self.bottom_silicon_sensors.append(sensor_active)
                self.bottom_silicon_sensors.append(sensor_periphery)
        ...
        self.cf_end_cut_holes = []

        # Bottom carbon fibre layer: cut only on the LEFT
        bottom_cf_y = pheight

        self.bottom_carbon_fiber = mt.createRectangle(
        start=[self.edge_clearance, bottom_cf_y],
        end=[self.model_length, bottom_cf_y + self.cfthickness],
        boundaryMarker=[9, 10, 11, 12],
        marker=1,
        area=0.01
        )
        ...
        # Foam layer created in separate pieces instead of using holes
        foam_bottom = pheight
        half_foam = self.fthickness / 2.0

        self.foam_parts = []

        # Left end: keep only the upper half of foam
        self.foam_parts.append(
        mt.createRectangle(
                start=[0.0, foam_bottom + half_foam],
                end=[self.edge_clearance, foam_bottom + self.fthickness],
                boundaryMarker=[13, 14, 15, 16],
                marker=2,
                area=0.01
        )
        )

        # Middle full foam section
        self.foam_parts.append(
        mt.createRectangle(
                start=[self.edge_clearance, foam_bottom],
                end=[self.model_length - self.edge_clearance, foam_bottom + self.fthickness],
                boundaryMarker=[13, 14, 15, 16],
                marker=2,
                area=0.01
        )
        ...
        # Right end: keep only the lower half of foam
        self.foam_parts.append(
        mt.createRectangle(
                start=[self.model_length - self.edge_clearance, foam_bottom],
                end=[self.model_length, foam_bottom + half_foam],
                boundaryMarker=[13, 14, 15, 16],
                marker=2,
                area=0.01
        )
        ...
        # Top carbon fibre layer, with both ends removed

        # Top carbon fibre layer: cut only on the RIGHT
        top_cf_y = pheight

        self.top_carbon_fiber = mt.createRectangle(
        start=[0.0, top_cf_y],
        end=[self.model_length - self.edge_clearance, top_cf_y + self.cfthickness],
        boundaryMarker=[17, 18, 19, 20],
        marker=1,
        area=0.01
        )
        ...
        for x0 in self.top_silicon_x:
                ...
                glue = mt.createRectangle(
                        ...
                        pheight + self.si_glue_thickness],
                ...
                sensor_periphery = mt.createRectangle(
                        start=[x0 + active_length,
                        pheight + self.si_glue_thickness],
                        end=[x0 + active_length + periphery_length,
                        pheight + self.si_glue_thickness + self.si_thickness],
                        boundaryMarker=[201, 202, 203, 204],
                        marker=18,
                        area=0.01
                ...
                self.top_silicon_sensors.append(sensor_active)
                self.top_silicon_sensors.append(sensor_periphery)
        ...
        # Final geometry assembly
        self.geom = self.bottom_carbon_fiber + self.top_carbon_fiber

        for foam_part in self.foam_parts:
            self.geom += foam_part
...
#     def export_geometry_and_mesh(self):
#         """Export geometry and mesh for external visualization."""
#         exportfolder = outputfolder + "exports/"
#         os.makedirs(exportfolder, exist_ok=True)

#         # Export mesh to VTK for ParaView
#         self.mesh.exportVTK(
#                 f"{exportfolder}/{self.modelname}_mesh.vtk"
#         )

#         # Export PLC geometry
#         mt.exportPLC(
#                 self.geom,
#                 f"{exportfolder}/{self.modelname}_geometry.poly"
#         )

#         # Export mesh to HDF5
#         mt.exportHDF5Mesh(
#                 self.mesh,
#                 f"{exportfolder}/{self.modelname}_mesh.h5"
#         )

#         # Export surface mesh to STL
#         mt.exportSTL(
#                 self.mesh,
#                 f"{exportfolder}/{self.modelname}_surface.stl"
#         )

#         print("Exported mesh and geometry files to:", exportfolder)
    ...
    def study_solve(self):
        ...
        # }
        ###############################################################################
        # Check mesh markers before solving

        markers = sorted(set(c.marker() for c in self.mesh.cells()))
        print("Cell markers present in mesh:", markers)

        if 0 in markers:
            raise RuntimeError(
                "Marker 0 exists in the mesh. Some cut/hole region is still meshed without material."
        )

        boundary_markers = sorted(set(b.marker() for b in self.mesh.boundaries()))
        print("Boundary markers present in mesh:", boundary_markers)

        if 99 not in boundary_markers:
                raise RuntimeError(
                        "Boundary marker 99 is missing. The CO2 cooling boundary is not being applied."
                )
        ...
        for cell in self.mesh.cells():
                ...
                # Active silicon
                if cell.marker() in [8, 9]:
                        source[cell.id()] = (
                        self.powerdensity / self.si_thickness
                        )

                # Periphery silicon
                elif cell.marker() in [18, 19]:
                        source[cell.id()] = (
                        self.periphery_powerdensity /
                        self.si_thickness
                        )
        ...
        print("Marker 8 cells  =", n8)
        print("Marker 18 cells =", n18)
        print("Marker 9 cells  =", n9)
        print("Marker 19 cells =", n19)
    ...
    f=source
        ...
        if not np.all(np.isfinite(u)):
                print("NaN/Inf detected in solution")
                print("Number of bad values:", np.sum(~np.isfinite(u)))

                bad_ids = np.where(~np.isfinite(u))[0]
                print("First bad node IDs:", bad_ids[:20])
